# Offer and pricing

Source: Cost Model V10.10, rebuilt 4 September 2026, chapter logic settled 15 September 2026.
Sheet: `1vN5YckOTYgne0yppRkfm2jYBjPp1ff6ug8gWntLeS8k`

This supersedes the Bronze / Silver / Gold tiers in `README.md`, and it supersedes the earlier £1,800 / £2,500 / £3,500 / £4,000 ladder.

## The ladder, one parent (LIST, per parent)

| Option | What they're buying | LIST | Standalone value |
|---|---|---|---|
| A | Solo, no child interview | £2,400 | £7,000 |
| B | Solo, buyer interviewed | £3,150 | £8,500 |
| C | Children batched, one session | £3,350 | £11,000 |
| D | Children individually, 2 children baseline | £4,000 | £13,000 |

Option D adds £300 per child above the baseline of 2. Value uplift £1,000 per extra child.

## Two parents (LIST)

| Option | Children | LIST |
|---|---|---|
| A | 1 | £4,700 |
| B | 1 | £6,200 |
| C | 2 to 4 | £6,600 |
| D | 2 | £7,800 |
| D | 3 | £8,400 |
| D | 4 | £9,000 |

Every two-parent LIST is exactly 2.5% off double the one-parent LIST, rounded up to the nearest £100. Say this to families: two-parent pricing includes a flat 2.5% multi-parent saving versus two separate documentaries.

## What families actually see these called

The A to D labels are internal. In proposals they are named:

| Internal | Client-facing name |
|---|---|
| A and B | Hero Story |
| C | Family Documentary |
| D | Full Chorus, or The Full Chorus |

Option B is sold as Hero Story with the buyer's own hour added, not as a fourth shape. Confirmed against the Catlin proposal of 15 September 2026 and the Sturgess proposal of 9 September 2026.

Every proposal also carries the guarantee: full refund if it doesn't feel right after the kickoff call and before the first recorded session. Thirty-five families, never invoked.

## Bolt-ons appearing in proposals but not in the cost model

These were offered at a discount off a stated "usually" price. Neither the usual price nor the margin is in V10.10, so they need adding to the model.

| Bolt-on | Offered at | Stated usual |
|---|---|---|
| Mementos package, 3 audio clips plus 1 video memento, per parent | £350 | £550 |
| Snapshot, one 1.5-hour session with the children | £250 | £400 |
| Grandchildren, up to 30 minutes raw | £200 | £400 |

Note the clash: the model prices Grandchildren at £300, the Sturgess proposal offered it at £200 against a usual of £400. Resolve before the next proposal goes out.

## Price band rules

- TARGET is 5% off LIST. That is where most deals are designed to land, not conceded to.
- FLOOR is 7.5% off LIST. The walk-away line. Never below.
- Minimum acceptable margin on a full-cost basis is 50%.
- A second parent is priced at 95% of the first.

Margins at LIST run 60% to 66% across the board, so there is room to move inside the band.

## Bolt-ons

| Bolt-on | Price | Margin |
|---|---|---|
| Grandchildren, 30 minutes recorded and played to the parent live | £300 | 67.5% |
| Additional 2-hour session, a 4th parent session or a new voice | £750 | 62.3% |
| Extra child on option D | £300 | 52.9% |
| Stories We Tell, one 1-hour session, Riverside straight out | £200 | 53.8% |

## Chapters

One chapter per recorded strand, plus one for the wrap-up.

- Parent's own 3-session set: 6, times parents recorded
- Buyer's own hour, option B only: 1, times parents recorded
- Children's group sitting, option C only: 1 per family, does not double for two parents
- Each child's own session, option D only: 1 per child per parent
- Family wrap-up: 1 per family, options B C and D, never A

Totals, one parent: A 6, B 8, C 8, D 9 to 11. Two parents: A 12, B 15, C 14, D 17 to 21.

## Open items flagged in the model

1. Group-session editor rates are stand-ins and need confirming with Vuk and Abhi.
2. Clips and audiograms are not costed at all. Needs writing into the editor brief per chapter, then a price agreed.
3. Overheads, tools and subscriptions are not yet real numbers, so profit and cash lines read too good.
4. Overages are not in the standard prices. Jodie charges £1.50 per minute of raw beyond 6 hours.
